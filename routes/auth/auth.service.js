...
module.exports = {
  ...
  getUserById: (user_id, callback) => {
    const sql = `select user_id, userName, email, phoneNo, vpa, password from users where user_id="${user_id}"`;
  ...
  getUserByEmail: (email, callback) => {
    const sql = `select user_id, userName, email, phoneNo, vpa, password from users where email="${email}"`;
    pool.query(sql, (err, result) => {
      if (err) {
        return callback(err);
      }
