...
module.exports = {
  ...
  getBillMembersByBillId: (bill_id, callback) => {
    const sql = `select B.bill_id, B.name, B.expense as total_expense, B.created_date, B.created_by, U1.user_id, U1.userName, U1.phoneNo, U1.email, U2.user_id as owner_user_id, U2.userName as owner_userName, U2.phoneNo as owner_phoneNo, U2.email as owner_email, U2.vpa as owner_vpa, UG.txn_id, UG.guestName, UG.status, UG.paid_date, UG.owes_to, UG.expense as amount_to_pay
				from user_groups UG
				left join users U1 
  ...
  getReceivedPaymentRecords: (user_id, callback) => {
    const sql = `select B.bill_id, B.name, B.expense as total_expense, B.created_date, B.created_by, U1.user_id, U1.userName, U1.phoneNo, U1.email, U2.user_id as owner_user_id, U2.userName as owner_userName, U2.phoneNo as owner_phoneNo, U2.email as owner_email, UG.txn_id, UG.guestName, UG.status, UG.paid_date, UG.owes_to, UG.expense as amount_to_pay
					from user_groups UG
					left join users U1 
					on UG.user_id=U1.user_id
					join users U2 on UG.owes_to=U2.user_id AND UG.owes_to="${user_id}" 
					join bills B
					on UG.bill_id=B.bill_id
  				`;
    pool.query(sql, (err, result) => {
